...
static int output_haptic_feedback_enable(const struct device *dev, uint8_t force) {
...
static int output_haptic_feedback_disable(const struct device *dev) {
    struct output_haptic_feedback_data *data = dev->data;
    const struct output_haptic_feedback_config *config = dev->config;
    ...
    if (config->fb_drv == OUTPUT_HAPTIC_FEEDBACK_DRIVER_DRV2605) {
        ...
        if (err) {
            LOG_WRN("Fail to sensor_attr_set DRV2605_ATTR_STOP");
            rc = -EIO;
            goto exit;
        }
    }
...
static int output_haptic_feedback_get(const struct device *dev) {
    struct output_haptic_feedback_data *data = dev->data;
    return !data->busy;
}
...
static const struct output_generic_api api = {
    .enable = output_haptic_feedback_enable,
    .disable = output_haptic_feedback_disable,
    .get = output_haptic_feedback_get,
};
